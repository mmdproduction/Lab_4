...
class Exception : public std::exception {
    ...
    std::string message;
    public:
    Exception(const char* func_name,  const char* file_name, uint32_t line, const std::string& msg = "Неизвестная ошибка"): message(std::format("{} Функция: {} Файл: {}:{}", msg, func_name, file_name, line )){}
...
class InvalidNextValue : public Exception{
    public:
    InvalidNextValue( const std::source_location& loc = std::source_location::current())
...
class EndOfStream : public Exception{
    public:
    EndOfStream( const std::source_location& loc = std::source_location::current())
        : Exception(loc.function_name(), loc.file_name(), loc.line(), "Конец потока чтения!"){}
};

class EmptyStream : public Exception{
    public:
    EmptyStream( const std::source_location& loc = std::source_location::current())
